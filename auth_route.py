...
async def create_conta(email: str, senha: str, username: str, session = Depends(CrearSession)): #antes de hacer el archivo 'dependencias.py', los parametros eran solo los primeros 3, despues de crear el archivo puedo pasar el parametro session que hace referencia al return de la funcion create_session
    ...
    if len(usuario) > 0:
        ...
        senha_nova = bcrypt_context.hash(senha)
        NewUser = User(username, email, senha_nova)
